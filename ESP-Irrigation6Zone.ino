...
const uint8_t valveChannel[Zone] = { P0, P1, P2, P3, P4, P5 }; // Use next available pins
...
float  lastRainAmount = 0.0f; // Add at top with globals
...
void turnOnZone(int z) {
  ...
  logEvent(z, "START", "-", false); // No more source arg
...
void handleSubmit() {
  for (int z = 0; z < Zone; z++) {
    ...
    // Enable second start time
    enableStartTime2[z] = server.hasArg("enableStartTime2" + String(z));
  }

  saveSchedule();
  updateCachedWeather();  // (optional: update weather cache on every submit)
...
void handleSetupPage() {
  ...
  // --- JS: Dark mode, simple validation ---
  html += R"rawliteral(
  <script>
    function setTheme(theme) {
      if(theme==='dark') {
