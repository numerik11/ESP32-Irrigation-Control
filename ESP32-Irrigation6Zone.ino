...
void setup() {
  ...
  // Status JSON (drop-in replacement)
  ...
  // Wi-Fi strength
  long rssi = (WiFi.status() == WL_CONNECTED) ? WiFi.RSSI() : -127;   // dBm
  // Map roughly: -90 dBm → 0%, -50 dBm → 100%
  int rssiPct = constrain(map((int)rssi, -90, -50, 0, 100), 0, 100);
  doc["rssi"]    = rssi;
  doc["rssiPct"] = rssiPct;

  // Tank level (calibrated)
  int tankRaw = analogRead(TANK_PIN);
  int tankPct = constrain(map(tankRaw, tankEmptyRaw, tankFullRaw, 0, 100), 0, 100);
  int lowThresh = tankEmptyRaw + (int)((tankFullRaw - tankEmptyRaw) * 0.15f);
  bool tankLow = tankRaw <= lowThresh;
  doc["tankPct"] = tankPct;
  doc["tankLow"] = tankLow;
  ...
  for (int i = 0; i < Zone; i++) {
    ...
    // Source string (kept for UI; not used to drive logic)
    const char* src = "Tank";
    if (justUseMains)      src = "Mains";
    else if (justUseTank)  src = "Tank";
    else                   src = (tankLow ? "Mains" : "Tank");
    z["source"] = src;
  }
...
void handleRoot() {
  ...
  // Tank (initial snapshot; JS will keep it fresh)
  int tankRaw = analogRead(TANK_PIN);
...
.summary-row { display: flex; flex-wrap: wrap; gap: 18px; justify-content: center; align-items: flex-start; width: 100%; margin-bottom: 18px; }
...
.footer-links { border-top: 1.5px solid #dae7f5; margin: 28px 0 0 0; padding-top:18px; text-align:center; font-size:1.09em; background: var(--footer); border-radius:0 0 12px 12px; transition:background .2s; }
...
<div class='container'>
  ...
  <div class='summary-row'>
    ...
    <div class='summary-block'><span class='icon'>📍</span><br>)";
  ...
  html += (isnan(temp) ? String("--") : String(temp, 1) + " ℃");
  html += R"(</div>
    <div class='summary-block'><span class='icon'>💧</span><br>)";
  ...
  for (int zone = 0; zone < Zone; zone++) {
    ...
    // manual buttons with ids so JS can enable/disable
    html += "<div class='manual-control-container'>"
            "<button type='button' id='btn-on-" + String(zone) + "'  class='turn-on-btn'  data-zone='" + String(zone) + "'>▶️ On</button>"
            "<span style='width:8px;display:inline-block;'></span>"
            "<button type='button' id='btn-off-" + String(zone) + "' class='turn-off-btn' data-zone='" + String(zone) + "'>⏹ Off</button>"
            "</div>";

    html += "</div>"; // .zone-container
  }

  html += "</div>" // .zones-wrapper
          "<div style='display:flex;gap:10px;justify-content:center;width:100%;margin-top:10px;'>"
            "<button type='submit'>Update Schedule</button>"
            "<button type='button' id='stop-all-btn' title='Turn off all running zones'>⏹ Stop All</button>"
          "</div>"
          "</form>";
...
function setDisabled(btn, disabled, labelIfBusy){
  if(!btn) return;
  btn.disabled = !!disabled;
  if(disabled && labelIfBusy){ btn.dataset._txt = btn.textContent; btn.textContent = labelIfBusy; }
  else if(btn.dataset._txt){ btn.textContent = btn.dataset._txt; delete btn.dataset._txt; }
}

// live status refresh (2s)
function updateLive(){
  fetch('/status')
    .then(r=>r.json())
    .then(st=>{
      ...
      if(wifiText) wifiText.textContent =
        (typeof st.rssi === 'number') ? `${st.rssi} dBm (${st.rssiPct}%)` : `-- dBm (--%)`;
      ...
      if(Array.isArray(st.zones)){
        st.zones.forEach((z, i)=>{
          ...
          if(rmEl){ rmEl.textContent = z.active ? `(${fmtMMSS(z.remaining)} left)` : ''; }

          const onBtn  = document.getElementById('btn-on-' + i);
          const offBtn = document.getElementById('btn-off-' + i);
          if(onBtn)  onBtn.disabled  = !!z.active;
          if(offBtn) offBtn.disabled = !z.active;

          if(z.active) anyActive = true;
        });
      }
...
// POST helper (disables buttons during request)
function postCmd(url, btn){
  setDisabled(btn, true, 'Working…');
  return fetch(url,{method:'POST'}).then(r=>r.text()).catch(()=>null).finally(()=>{
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  // Wire up per-zone buttons (no reloads)
  document.querySelectorAll('.turn-on-btn').forEach(btn=>{
    btn.addEventListener('click',()=> postCmd('/valve/on/'+btn.dataset.zone, btn));
  });
  document.querySelectorAll('.turn-off-btn').forEach(btn=>{
    btn.addEventListener('click',()=> postCmd('/valve/off/'+btn.dataset.zone, btn));
  });

  // Stop all
  const stopAll = document.getElementById('stop-all-btn');
  if(stopAll){
    stopAll.addEventListener('click',()=> postCmd('/stopall', stopAll));
  }

  // Backlight
  const tb=document.getElementById('toggle-backlight-btn');
  if(tb){tb.addEventListener('click',()=>{postCmd('/toggleBacklight', tb);});}
});
</script></body></html>)";
