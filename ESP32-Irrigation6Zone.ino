...
static const unsigned long WEATHER_INTERVAL = 3600000UL; // 1 hour (used everywhere)
...
// Globals
// -------------------------------
String apiKey, city;                 // city holds OpenWeather city ID (string)
...
const uint8_t I2C_HEALTH_DEBOUNCE = 5;
uint8_t i2cFailCount = 0;

// -------------------------------
// Prototypes (added missing)
// -------------------------------
void wifiCheck();
void checkI2CHealth();
...
void setup() {
  ...
  if (!wifiManager.autoConnect("ESPIrrigationAP")) {  // unified SSID
  ...
  while (now < 1000000000) { delay(300); now = time(nullptr); }

  // OTA & Web Server (hostname before begin)
...
void loop() {
  ...
  // Hard stop if weather delay is active
  ...
  // Start next queued if nothing running
  {
    bool someone = false;
    for (int i = 0; i < Zone; ++i) if (zoneActive[i]) { someone = true; break; }
    if (!someone) {
...
void checkI2CHealth() {
  ...
  for (auto addr : expanderAddrs) {
    ...
    if (Wire.endTransmission() != 0) { anyErr = true; break; }
  }
  if (anyErr) {
    i2cFailCount++;
    if (i2cFailCount >= I2C_HEALTH_DEBOUNCE) {
      ...
      for (uint8_t i = 0; i < Zone; i++) {
        ...
        digitalWrite(zonePins[i], LOW); // safe OFF on switchover (GPIO: HIGH=ON)
      }
    }
  } else {
    i2cFailCount = 0;
...
void initGpioFallback() {
  ...
  for (uint8_t i = 0; i < Zone; i++) {
    pinMode(zonePins[i], OUTPUT);
    digitalWrite(zonePins[i], LOW); // OFF
  }
}
...
void saveConfig() {
  ...
  for (int i = 0; i < Zone; i++) f.println(zonePins[i]);
  f.close();
}
...
void loadSchedule() {
  ...
  for (int i = 0; i < Zone; i++) {
    ...
    Serial.printf(
      "Zone %d: %02d:%02d, %02d:%02d, %d mins, %d sec, Days: %d%d%d%d%d%d%d\n",
      i+1,
...
void updateCachedWeather() {
  ...
  if (cachedWeatherData == "" || (currentMillis - lastWeatherUpdate >= WEATHER_INTERVAL)) {
    ...
    if (!deserializeJson(js, cachedWeatherData)) {
      if (js.containsKey("rain") && js["rain"].containsKey("1h"))
...
void HomeScreen() {
  ...
  updateCachedWeather();
  DynamicJsonDocument js(1024);
  if (!deserializeJson(js, cachedWeatherData)) { /* ok */ }
  ...
  bool low = raw < (tankEmptyRaw + (tankFullRaw - tankEmptyRaw) * 0.15f);
  ...
  // Lines 4–5: Zones 1–6 (3 per row)
  auto drawZone = [&](int i, int x, int y){
...
void updateLCDForZone(int zone) {
  ...
  String line1 = "Zone " + String(zone + 1) + " " + String(elapsed / 60) + ":" + (elapsed % 60 < 10 ? "0" : "") + String(elapsed % 60);
  String line2 = (elapsed < total) ? (String(rem / 60) + "m Remaining") : "Complete";

  display.clearDisplay();
  const uint8_t charW = 6;
  const uint8_t charH = 8;
  uint8_t maxCols = display.width() / charW;
...
bool checkWindRain() {
  ...
  // Rain: only if measured rain in the last hour
  if (rainDelayEnabled) {
    float r1h = 0.0f;
    if (js.containsKey("rain") && js["rain"].containsKey("1h")) {
      r1h = js["rain"]["1h"].as<float>();
    }
    rainActive = (r1h > 0.0f);
    if (rainActive) {
      Serial.printf("Rain-delay active (1h=%.2fmm)\n", r1h);
      return false;
    }
  } else {
    rainActive = false;
...
bool isTankLow() {
  int raw = analogRead(TANK_PIN);
  return raw < (tankEmptyRaw + (tankFullRaw - tankEmptyRaw) * 0.15f);
}
...
void handleTankCalibration() {
  ...
  html += "<style>body{font-family:'Roboto',sans-serif;background:#f0f4f8;margin:0;padding:0;display:flex;align-items:center;justify-content:center;height:100vh;}\
 .card{background:#fff;padding:30px;border-radius:10px;box-shadow:0 4px 6px rgba(0,0,0,0.1);width:100%;max-width:400px;text-align:center;}\
 h1{color:#007BFF;margin-bottom:20px;}p{margin:10px 0;font-size:1.1em;}form{margin:10px 0;}button{padding:10px 20px;border:none;border-radius:5px;background:#007BFF;color:#fff;font-size:16px;cursor:pointer;}button:hover{background:#0056b3;}a{display:block;margin-top:15px;color:#007BFF;text-decoration:none;}a:hover{text-decoration:underline;}.alert{color:#cc0000;font-weight:bold;}</style></head><body>";

  html += "<div class='card'><h1>Tank Calibration</h1>";
  ...
  html += "<form method='POST' action='/setTankFull'><button type='submit'>Set as Full</button></form>";
  html += "<a href='/setup'>⬅ Back to Setup</a><a href='/'>🏠 Home</a>";
...
void turnOnZone(int z) {
  ...
  if (useGpioFallback) digitalWrite(zonePins[z], HIGH); // ON (GPIO path)
  else                 pcfOut.digitalWrite(valveChannel[z], LOW); // ON (active-low)
...
void turnOffZone(int z) {
  ...
  else                 pcfOut.digitalWrite(valveChannel[z], HIGH); // OFF
...
void handleRoot() {
 ...
 <body>
  ...
  // Footer Links (city ID link fixed)
...
void handleSetupPage() {
    ...
    <div class="setup-card">
      <form id="setupForm" action="/configure" method="POST" autocomplete="off">
        ...
        R"rawliteral(" required maxlength="32" placeholder="City ID">
            <span class="tooltip"><span class="tip">OpenWeather city ID (numeric).</span></span>
          </div>
        </section>
        <section>
        ...
        R"rawliteral(" required placeholder="m/s">
          ...
          <div class="checkbox-group">
            ...
            <span class="tooltip"><span class="tip">Pause on rain (uses last 1h rain).</span></span>
          </div>
          <div class="checkbox-group">
  ...
  // --- JS: Dark mode & lightweight validation ---
  html += R"rawliteral(
  <script>
    function setTheme(theme) {
      if(theme==='dark') {
...
void handleLogPage() {  
  ...
  String html = "<!DOCTYPE html><html lang='en'><head>"
    ...
    "</form>"
    "<table>"
    "<tr><th>🕒 Timestamp</th><th>💧 Zone</th><th>⚡ Event</th><th>🔗 Source</th><th>🌧 Rain Delay</th><th>📋 Details</th></tr>";
